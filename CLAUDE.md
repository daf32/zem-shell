...
**What is a builtin and what is a plugin.** The core is the parser, the executor, job control, the variable model and the commands without which a shell is not a shell: `cd`, `pwd`, `exit`, `set`, `unset`, `export`, `get`, `jobs`/`fg`/`bg`/`kill`/`wait`/`disown`, plus `config` and `plugin` (turn those off and there is no way back). Everything optional ships as a plugin under `src/zem/plugins/`, a module or a package, listed by `plugin list` and switchable with `plugin disable`: `coreutils` (`echo` `printf` `test` `[` `true` `false` `:`), `scripting` (`source` `.` `eval` `exec` `read` `command` `type`), `dirstack` (`pushd` `popd` `dirs`), `history` (the command only), `theme` (with the bundled themes), `logo`, `venv`, `weather`.
