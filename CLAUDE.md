...
- `prompt.py` — the prompt as named segments (`venv`, `exit_code`, `path`, `git`, `symbol`, `duration`, `time`), ordered by `input.segments` / `input.rprompt_segments`. `PromptRenderer` holds them, plugins add or replace by name via `prompt_segments()`, and a segment that returns `None` (no venv, not a repo) or raises is skipped without costing the prompt. `colored=False` renders the same segments as plain text, rather than the separate implementation the text prompt used to have.
